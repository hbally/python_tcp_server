...
if __name__ == '__main__':
    ...
    # chat_from = random.choice(all_phone_numbers)
    # all_phone_numbers.remove(chat_from)

    reactor.callLater(3, cf.p.send_verify, chat_from)
    to = random.choice(all_phone_numbers)
    to = '000001'
    # 同时发送一样的指令 send_single_chat 导致后一个覆盖前一个指令的数据导致数据丢失
    reactor.callLater(20, cf.p.send_single_chat, chat_from, to, '你好,这是单聊1')
    reactor.callLater(20, cf.p.send_single_chat, chat_from, to, '你好,这是单聊2')
    # reactor.callLater(11, cf.p.send_group_chat, chat_from,
    #                   [random.choice(all_phone_numbers), random.choice(all_phone_numbers)], '你好,这是组聊')
